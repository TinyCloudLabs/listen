...
for (const entry of import_bootstrap.CAPABILITY_REGISTRY) {
...
function materialForUnsigned(unsigned, descriptor) {
  ...
  return {
    ...
    jcsBytes,
    digest,
    digestHex: (0, import_viem3.bytesToHex)(digest).slice(2)
  };
}
...
async function verifySignature(signature, digest) {
  if (signature.suite === ED25519_JCS_SIGNATURE_SUITE) {
    ...
    try {
      return import_ed25519.ed25519.verify(signatureBytes, digest, publicKey);
    } catch {
  ...
  if (signature.suite === EIP191_JCS_SIGNATURE_SUITE) {
    ...
    const signatureBytes = decodeSignatureValue(signature.value, signature.suite);
    try {
      return (0, import_viem3.verifyMessage)({
        address: pkh.address,
        message: { raw: digest },
        signature: (0, import_viem3.bytesToHex)(signatureBytes)
      });
    } catch {
...
function ed25519PublicKeyFromDidKey(did) {
  ...
  let bytes;
  try {
    bytes = import_basics.bases.base58btc.decode(identifier);
  } catch {
...
function sha256Bytes(bytes) {
  return (0, import_viem3.sha256)(bytes, "bytes");
}
...
var JsonValueSchema = import_zod2.z.lazy(
  () => import_zod2.z.union([
    ...
    import_zod2.z.record(JsonValueSchema)
  ])
);
var Rfc3339Schema = import_zod2.z.string().refine((value) => parseStrictRfc33392(value) !== void 0, {
  message: "must be strict RFC 3339 date-time with timezone"
});
var SignedRecordSchema = import_zod2.z.object({
  schema: import_zod2.z.string(),
  engineRecordId: import_zod2.z.string(),
  ownerDid: import_zod2.z.string(),
  endpoint: import_zod2.z.string(),
  audience: import_zod2.z.string(),
  supportedPolicyVersions: import_zod2.z.array(import_zod2.z.string()),
  supportedEvidenceVerifiers: import_zod2.z.array(import_zod2.z.string()),
  grantIssuerDid: import_zod2.z.string(),
  expiresAt: Rfc3339Schema,
  signature: import_zod2.z.object({
    suite: import_zod2.z.string(),
    signerDid: import_zod2.z.string(),
    value: import_zod2.z.string()
  }).strict()
}).strict();
var PolicyEngineSchema = import_zod2.z.object({
  endpoint: import_zod2.z.string().url(),
  audience: import_zod2.z.string(),
  supportedEvidenceVerifiers: import_zod2.z.tuple([import_zod2.z.literal(W3C_VC_CREDENTIAL_VERIFIER)]),
  signedRecord: SignedRecordSchema
}).strict();
var OwnerNodeSchema = import_zod2.z.object({
  schema: import_zod2.z.literal(OWNER_NODE_ENDPOINT_SCHEMA),
  endpoint: import_zod2.z.string().url(),
  spaceId: import_zod2.z.string().min(1)
}).strict();
var ResourceHintSchema = import_zod2.z.object({
  resourceType: import_zod2.z.string(),
  resourceId: import_zod2.z.string(),
  requestedCapabilities: import_zod2.z.array(JsonValueSchema).min(1)
}).strict();
var BootstrapSchema = import_zod2.z.object({
  schema: import_zod2.z.literal(TRANSCRIPT_SHARE_BOOTSTRAP_SCHEMA),
  policyId: import_zod2.z.string(),
  policyEngine: PolicyEngineSchema,
  ownerNode: OwnerNodeSchema,
  resourceHint: ResourceHintSchema
}).strict();
var SignatureSchema = import_zod2.z.object({
  suite: import_zod2.z.string(),
  signerDid: import_zod2.z.string(),
  value: import_zod2.z.string()
}).strict();
var ChallengeSchema = import_zod2.z.object({
  schema: import_zod2.z.literal(POLICY_ENGINE_CHALLENGE_RESPONSE_SCHEMA),
  challengeId: import_zod2.z.string(),
  policyId: import_zod2.z.string(),
  audience: import_zod2.z.string(),
  nonce: import_zod2.z.string().min(16),
  challengeExpiresAt: Rfc3339Schema,
  acceptedSuites: import_zod2.z.array(import_zod2.z.string()).min(1),
  requestedCapabilitiesTemplate: import_zod2.z.array(JsonValueSchema).optional(),
  signature: SignatureSchema
}).strict();
var ChallengeResponseSchema = import_zod2.z.object({ challenge: ChallengeSchema }).strict();
var DenialSchema = import_zod2.z.object({
  schema: import_zod2.z.literal(POLICY_ENGINE_DENIAL_SCHEMA),
  code: import_zod2.z.enum(POLICY_ENGINE_GRANT_PRESENTATION_DENIAL_CODES),
  message: import_zod2.z.string().optional()
}).strict();
var ErrorEnvelopeDenialSchema = import_zod2.z.object({
  error: import_zod2.z.object({
    code: import_zod2.z.enum(POLICY_ENGINE_GRANT_PRESENTATION_DENIAL_CODES),
    message: import_zod2.z.string().optional()
  }).strict()
}).strict();
var CapabilitySchema = import_zod2.z.object({
  service: import_zod2.z.enum(["tinycloud.kv", "tinycloud.sql", "tinycloud.vfs"]),
  space: import_zod2.z.string(),
  path: import_zod2.z.string(),
  actions: import_zod2.z.array(import_zod2.z.string()).min(1),
  caveats: JsonValueSchema.optional()
}).strict();
var WireDelegationSchema = import_zod2.z.object({
  delegationId: import_zod2.z.string(),
  issuerDid: import_zod2.z.string(),
  holderDid: import_zod2.z.string(),
  policyId: import_zod2.z.string(),
  capabilities: import_zod2.z.array(CapabilitySchema).min(1).optional(),
  issuanceId: import_zod2.z.string().optional(),
  capabilityHashHex: import_zod2.z.string().optional(),
  revocationMode: import_zod2.z.literal("refresh_only").optional(),
  issuedAt: Rfc3339Schema,
  expiresAt: Rfc3339Schema,
  terminal: import_zod2.z.boolean(),
  encoded: import_zod2.z.string()
}).strict();
var ResolveResponseSchema = import_zod2.z.object({ delegation: WireDelegationSchema }).strict();
var DelegateReceiptSchema = import_zod2.z.object({ cid: import_zod2.z.string().min(1), activated: import_zod2.z.array(import_zod2.z.string()), skipped: import_zod2.z.array(import_zod2.z.string()) }).strict();
var SqlReadResponseSchema = import_zod2.z.object({ rows: import_zod2.z.array(JsonValueSchema) }).strict();
var KvReadResponseSchema = import_zod2.z.object({ value: JsonValueSchema }).strict();
...
var EnsDataSchema2 = import_zod3.z.object({
  /** ENS name/domain. */
  domain: import_zod3.z.string().nullable().optional(),
  /** ENS avatar URL. */
  avatarUrl: import_zod3.z.string().nullable().optional()
});
var PersistedTinyCloudSessionSchema = import_zod3.z.object({
  /** The delegation header containing the UCAN */
  delegationHeader: import_zod3.z.object({
    Authorization: import_zod3.z.string()
  }),
  /** The delegation CID */
  delegationCid: import_zod3.z.string(),
  ...
  spaces: import_zod3.z.record(import_zod3.z.string(), import_zod3.z.string()).optional(),
  /** The verification method DID */
  verificationMethod: import_zod3.z.string()
});
var PersistedSessionDataSchema = import_zod3.z.object({
  /** User's Ethereum address */
  address: import_zod3.z.string().regex(ethereumAddressPattern, "Invalid Ethereum address"),
  /** EIP-155 Chain ID */
  chainId: import_zod3.z.number().int().positive(),
  /** Session key in JWK format (stringified) */
  sessionKey: import_zod3.z.string(),
  ...
  expiresAt: import_zod3.z.string().datetime({ offset: true }),
  /** Session creation timestamp (ISO 8601 with timezone offset) */
  createdAt: import_zod3.z.string().datetime({ offset: true }),
   ...
   */
  tinycloudHosts: import_zod3.z.array(import_zod3.z.string()).optional()
});
var TinyCloudSessionSchema = import_zod3.z.object({
  /** User's Ethereum address */
  address: import_zod3.z.string().regex(ethereumAddressPattern, "Invalid Ethereum address"),
  /** EIP-155 Chain ID */
  chainId: import_zod3.z.number().int().positive(),
  ...
  jwk: import_zod3.z.object({}).passthrough(),
  /** The signed SIWE message */
  siwe: import_zod3.z.string(),
  /** User's signature of the SIWE message */
  signature: import_zod3.z.string()
...
var JWKSchema = import_zod4.z.object({
  /** Key type (e.g., "EC", "RSA", "OKP") */
  kty: import_zod4.z.string(),
  /** Curve for EC/OKP keys (e.g., "P-256", "Ed25519") */
  crv: import_zod4.z.string().optional(),
  /** X coordinate for EC keys, public key for OKP */
  x: import_zod4.z.string().optional(),
  /** Y coordinate for EC keys */
  y: import_zod4.z.string().optional(),
  /** Private key value (d parameter) */
  d: import_zod4.z.string().optional(),
  /** Public exponent for RSA keys */
  e: import_zod4.z.string().optional(),
  /** Modulus for RSA keys */
  n: import_zod4.z.string().optional(),
  /** Key ID */
  kid: import_zod4.z.string().optional(),
  /** Algorithm */
  alg: import_zod4.z.string().optional(),
  /** Key use (e.g., "sig", "enc") */
  use: import_zod4.z.string().optional(),
  /** Key operations (e.g., ["sign", "verify"]) */
  key_ops: import_zod4.z.array(import_zod4.z.string()).optional()
});
var KeyTypeSchema = import_zod4.z.enum(["main", "session", "ingested"]);
var KeyInfoSchema = import_zod4.z.object({
...
var DelegationErrorSchema = import_zod4.z.object({
  /** Error code for programmatic handling */
  code: import_zod4.z.string(),
  /** Human-readable error message */
  message: import_zod4.z.string(),
  /** The service that produced the error */
  service: import_zod4.z.literal("delegation"),
  /** Original error if wrapping another error */
  cause: import_zod4.z.instanceof(Error).optional(),
...
var DelegationSchema = import_zod4.z.object({
  /** Content identifier (CID) of the delegation */
  cid: import_zod4.z.string(),
  /** DID of the delegate (the party receiving the delegation) */
  delegateDID: import_zod4.z.string(),
  ...
  parentCid: import_zod4.z.string().optional(),
  /** Whether sub-delegation is allowed */
  allowSubDelegation: import_zod4.z.boolean().optional(),
  /** Authorization header (UCAN bearer token) */
  authHeader: import_zod4.z.string().optional()
});
var CapabilityEntrySchema = import_zod4.z.object({
  ...
  keys: import_zod4.z.array(KeyInfoSchema),
  /** The delegation that grants this capability */
  delegation: DelegationSchema,
  /** When this capability expires (accepts Date or ISO string from JSON) */
  expiresAt: import_zod4.z.coerce.date().optional()
});
var DelegationRecordSchema = import_zod4.z.object({
  /** Content identifier (CID) of the delegation */
  cid: import_zod4.z.string(),
  /** Space ID this delegation applies to */
  spaceId: import_zod4.z.string(),
...
var CreateDelegationParamsSchema = import_zod4.z.object({
  /** DID of the delegate (the party receiving the delegation) */
  delegateDID: import_zod4.z.string(),
  /** Resource path this delegation grants access to */
  path: import_zod4.z.string(),
  ...
  disableSubDelegation: import_zod4.z.boolean().optional(),
  /** Optional statement for the SIWE message */
  statement: import_zod4.z.string().optional()
});
var DelegationChainSchema = import_zod4.z.array(DelegationSchema);
var DelegationChainV2Schema = import_zod4.z.object({
  /** The root delegation from the original authority */
  root: DelegationSchema,
  /** Intermediate delegations in the chain (may be empty) */
  chain: import_zod4.z.array(DelegationSchema),
  /** The final delegation to the current user */
  leaf: DelegationSchema
});
var DelegationDirectionSchema = import_zod4.z.enum(["granted", "received", "all"]);
var DelegationFiltersSchema = import_zod4.z.object({
  ...
  actions: import_zod4.z.array(import_zod4.z.string()).optional(),
  /** Include revoked delegations */
  includeRevoked: import_zod4.z.boolean().optional(),
  /** Filter by delegator DID */
  delegator: import_zod4.z.string().optional(),
  /** Filter by delegatee DID */
  delegatee: import_zod4.z.string().optional(),
  /** Only include delegations valid at this time */
  validAt: import_zod4.z.coerce.date().optional(),
  /** Maximum number of results to return */
  limit: import_zod4.z.number().optional(),
  /** Cursor for pagination */
  cursor: import_zod4.z.string().optional()
});
var SpaceOwnershipSchema = import_zod4.z.enum(["owned", "delegated"]);
var SpaceInfoSchema = import_zod4.z.object({
  /** Space identifier */
  id: import_zod4.z.string(),
...
var ShareSchemaSchema = import_zod4.z.enum(["base64", "compact", "ipfs"]);
var ShareLinkSchema = import_zod4.z.object({
...
var ShareLinkDataSchema = createShareLinkDataSchema(import_zod4.z.unknown());
var IngestOptionsSchema = import_zod4.z.object({
  /** Whether to persist the delegation to storage */
  persist: import_zod4.z.boolean().optional(),
  /** Whether to validate the full delegation chain */
  validateChain: import_zod4.z.boolean().optional(),
  /** Name for the ingested key */
  keyName: import_zod4.z.string().optional(),
  /** Whether to create a session key for this delegation */
  createSessionKey: import_zod4.z.boolean().optional(),
  /** Override the priority for the ingested key */
  priority: import_zod4.z.number().optional()
});
var GenerateShareParamsSchema = import_zod4.z.object({
...
var DelegationManagerConfigSchema = import_zod4.z.object({
  /** TinyCloud host URLs */
  hosts: import_zod4.z.array(import_zod4.z.string()),
  /** Active session for authentication */
  session: import_zod4.z.unknown().refine(
    (val) => val !== null && typeof val === "object",
    { message: "Expected a ServiceSession object" }
  ),
  /** Platform-specific invoke function */
  invoke: import_zod4.z.unknown().refine(
    (val) => typeof val === "function",
    { message: "Expected an invoke function" }
  ),
  /** Optional custom fetch implementation */
  fetch: import_zod4.z.unknown().refine(
    (val) => val === void 0 || typeof val === "function",
    { message: "Expected a fetch function or undefined" }
  ).optional()
});
var KeyProviderSchema = import_zod4.z.object({
  /** Generate a new session key, returns key ID */
  createSessionKey: import_zod4.z.unknown().refine(
    (val) => typeof val === "function",
    { message: "Expected a function" }
  ),
  /** Get JWK for a key */
  getJWK: import_zod4.z.unknown().refine(
    (val) => typeof val === "function",
    { message: "Expected a function" }
  ),
  /** Get DID for a key */
  getDID: import_zod4.z.unknown().refine(
    (val) => typeof val === "function",
    { message: "Expected a function" }
  )
});
var DelegationApiResponseSchema = import_zod4.z.object({
  /** SIWE message content */
  siwe: import_zod4.z.string(),
  /** Signature of the SIWE message */
  signature: import_zod4.z.string(),
  /** Delegation version */
  version: import_zod4.z.number(),
  /** CID of the created delegation */
  cid: import_zod4.z.string().optional()
});
var DelegatedResourceSchema = import_zod4.z.object({
...
var CreateDelegationWasmParamsSchema = import_zod4.z.object({
   ...
   * ```
   */
  abilities: import_zod4.z.record(import_zod4.z.string(), import_zod4.z.record(import_zod4.z.string(), import_zod4.z.array(import_zod4.z.string()))),
  /** Expiration time in seconds since Unix epoch */
  expirationSecs: import_zod4.z.number(),
  /** Optional not-before time in seconds since Unix epoch */
  notBeforeSecs: import_zod4.z.number().optional()
});
var CreateDelegationWasmResultSchema = import_zod4.z.object({
  /** Base64url-encoded UCAN delegation */
  delegation: import_zod4.z.string(),
   ...
   */
  resources: import_zod4.z.array(DelegatedResourceSchema)
});

// src/spaces/spaces.schema.ts
var SpaceConfigSchema = import_zod5.z.object({
  /** The space identifier (full URI) */
  id: import_zod5.z.string(),
  /** The short name of the space */
  name: import_zod5.z.string(),
  /** Factory function to create a space-scoped KV service */
  createKV: import_zod5.z.function(),
  /** Factory function to create a space-scoped Data Vault service */
  createVault: import_zod5.z.function(),
  /** Optional factory function to create a space-scoped secrets service */
  createSecrets: import_zod5.z.function().optional(),
  /** Factory function to create space-scoped delegations */
  createDelegations: import_zod5.z.function(),
  /** Factory function to create space-scoped sharing */
  createSharing: import_zod5.z.function(),
  /** Function to get space info */
  getInfo: import_zod5.z.function()
});
var SpaceServiceConfigSchema = import_zod5.z.object({
  /** TinyCloud host URLs */
  hosts: import_zod5.z.array(import_zod5.z.string()),
  /** Active session for authentication */
  session: import_zod5.z.unknown(),
  /** Platform-specific invoke function */
  invoke: import_zod5.z.function(),
  /** Optional custom fetch implementation */
  fetch: import_zod5.z.function().optional(),
  /** Optional capability key registry for delegated space discovery */
  capabilityRegistry: import_zod5.z.unknown().optional(),
  /** Factory function to create a space-scoped KV service */
  createKVService: import_zod5.z.function().optional(),
  /** Factory function to create a space-scoped Data Vault service */
  createVaultService: import_zod5.z.function().optional(),
  /** Factory function to create a space-scoped secrets service */
  createSecretsService: import_zod5.z.function().optional(),
  /** User's PKH DID (derived from address or provided explicitly) */
  userDid: import_zod5.z.string().optional(),
  /** Optional SharingService for v2 sharing links (client-side) */
  sharingService: import_zod5.z.unknown().optional(),
  /** Factory function to create delegations using SIWE-based flow */
  createDelegation: import_zod5.z.function().optional()
});
var SpaceDelegationParamsSchema = CreateDelegationParamsSchema.extend({
  /** The space ID to create the delegation for */
  spaceId: import_zod5.z.string()
});
var ServerDelegationInfoSchema = import_zod5.z.object({
  /** DID of the delegator */
  delegator: import_zod5.z.string(),
  /** DID of the delegate */
  delegate: import_zod5.z.string(),
  /** Parent delegation CIDs - accepts string or byte array format from server */
  parents: import_zod5.z.array(import_zod5.z.union([import_zod5.z.string(), import_zod5.z.array(import_zod5.z.number())])),
  /** Expiration time (ISO8601 string) */
  expiry: import_zod5.z.string().optional(),
  /** Not-before time (ISO8601 string) */
  not_before: import_zod5.z.string().optional(),
  /** Issued-at time (ISO8601 string) */
  issued_at: import_zod5.z.string().optional(),
  /** Capabilities granted by this delegation */
  capabilities: import_zod5.z.array(
    import_zod5.z.object({
      resource: import_zod5.z.string(),
      ability: import_zod5.z.string()
    })
  )
});
var ServerDelegationsResponseSchema = import_zod5.z.record(
  import_zod5.z.string(),
  ServerDelegationInfoSchema
);
var ServerOwnedSpaceSchema = import_zod5.z.object({
  /** Space identifier */
  id: import_zod5.z.string(),
  /** Space name (optional, can be derived from id) */
  name: import_zod5.z.string().optional(),
  /** Owner DID */
  owner: import_zod5.z.string(),
  /** Creation timestamp */
  createdAt: import_zod5.z.string().optional()
});
var ServerOwnedSpacesResponseSchema = import_zod5.z.array(ServerOwnedSpaceSchema);
var ServerCreateSpaceResponseSchema = import_zod5.z.object({
  /** Space identifier */
  id: import_zod5.z.string(),
  /** Space name */
  name: import_zod5.z.string(),
...
var ServerSpaceInfoResponseSchema = import_zod5.z.object({
  /** Space identifier */
  id: import_zod5.z.string(),
  /** Space name (optional) */
  name: import_zod5.z.string().optional(),
  ...
  async ensureAccountIndex(db) {
    const migrated = await db.migrations.apply({
      ...
      migrations: [
        {
          id: "001_initial",
          sql: [...import_bootstrap2.ACCOUNT_INDEX_SCHEMA]
        }
      ]
    });
...
var EncodedShareDataSchema = import_zod6.z.object({
  ...
  /** DID of the key */
  keyDid: import_zod6.z.string().min(1, "keyDid is required"),
  /** The delegation granting access */
  delegation: DelegationSchema,
  /** Resource path this link grants access to */
  path: import_zod6.z.string().min(1, "path is required"),
  /** TinyCloud host URL */
  host: import_zod6.z.string().url("host must be a valid URL"),
  /** Space ID */
  spaceId: import_zod6.z.string().min(1, "spaceId is required"),
  /** Schema version (must be 1) */
  version: import_zod6.z.literal(1)
});
var ReceiveOptionsSchema = import_zod6.z.object({
  /**
   * Whether to automatically create a sub-delegation to the current session key.
   * Default: true
   */
  autoSubdelegate: import_zod6.z.boolean().optional(),
  /**
   * Whether to use the current session key for operations (requires autoSubdelegate).
   * Default: true
   */
  useSessionKey: import_zod6.z.boolean().optional(),
  /**
   * Ingestion options passed to CapabilityKeyRegistry.
   */
  ingestOptions: IngestOptionsSchema.optional()
});
var SharingServiceConfigSchema = import_zod6.z.object({
  /** TinyCloud host URLs */
  hosts: import_zod6.z.array(import_zod6.z.string().url()).min(1, "At least one host URL is required"),
  /**
   * Active session for authentication.
   * Required for generate(), optional for receive().
   */
  session: import_zod6.z.unknown().refine(
    (val) => val === void 0 || val !== null && typeof val === "object",
    { message: "Expected a ServiceSession object or undefined" }
  ).optional(),
  /** Platform-specific invoke function */
  invoke: import_zod6.z.unknown().refine((val) => typeof val === "function", {
    message: "Expected an invoke function"
  }),
  /** Optional custom fetch implementation */
  fetch: import_zod6.z.unknown().refine(
    (val) => val === void 0 || typeof val === "function",
    { message: "Expected a fetch function or undefined" }
  ).optional(),
  ...
  registry: import_zod6.z.unknown().refine(
    (val) => val !== null && typeof val === "object",
   ...
   * Session expiry time.
   */
  sessionExpiry: import_zod6.z.date().optional(),
  /**
   ...
   */
  onRootDelegationNeeded: import_zod6.z.unknown().refine((val) => val === void 0 || typeof val === "function", {
...
function base64UrlDecode2(encoded) {
...
var SharingService = class {
  ...
  encodeLink(data, schema = "base64") {
    ...
    const encoded = base64UrlEncode2(jsonString);
  ...
  decodeLinkWithValidation(link) {
    ...
    let jsonString;
    try {
      jsonString = base64UrlDecode2(base64Data);
    } catch (err6) {
      return {
        ok: false,
        error: createError2(
...
async function signLocationRecord(payload, signer) {
  ...
  const signature = signer.type === "did:pkh" ? await signer.signMessage(message) : base64UrlEncode3(
...
async function verifyPkhSignature(did, payload, signature) {
  ...
  return (0, import_viem5.verifyMessage)({
    address,
    message: payload,
    signature
  });
...
function verifyDidKeySignature(did, payload, signature) {
  ...
  return import_ed255192.ed25519.verify(
    signatureBytes,
...
function verifyDidKeyEd25519Signature(did, payload, signature) {
  const publicKey = ed25519PublicKeyFromDidKey2(did);
  return import_ed255192.ed25519.verify(signature, payload, publicKey);
}
function ed25519PublicKeyFromDidKey2(did) {
  ...
  const bytes = import_basics2.bases.base58btc.decode(identifier);
  if (bytes.length === 34 && bytes[0] === 237 && bytes[1] === 1) {
    return bytes.slice(2);
  }
...
function canonicalizeEntryMatches(requested, granted) {
  ...
  if (!pathContains2(granted.path, requested.path)) {
    return false;
  }
...
function pathContains2(grantedPath, requestedPath) {
