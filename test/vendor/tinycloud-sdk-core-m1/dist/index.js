...
var Rfc3339Schema = z2.string().refine((value) => parseStrictRfc33392(value) !== void 0, {
  message: "must be strict RFC 3339 date-time with timezone"
});
var SignedRecordSchema = z2.object({
  schema: z2.string(),
  engineRecordId: z2.string(),
  ownerDid: z2.string(),
  endpoint: z2.string(),
  audience: z2.string(),
  supportedPolicyVersions: z2.array(z2.string()),
  supportedEvidenceVerifiers: z2.array(z2.string()),
  grantIssuerDid: z2.string(),
...
var PolicyEngineSchema = z2.object({
  endpoint: z2.string().url(),
  audience: z2.string(),
  supportedEvidenceVerifiers: z2.tuple([z2.literal(W3C_VC_CREDENTIAL_VERIFIER)]),
  signedRecord: SignedRecordSchema
}).strict();
var OwnerNodeSchema = z2.object({
  schema: z2.literal(OWNER_NODE_ENDPOINT_SCHEMA),
  endpoint: z2.string().url(),
  spaceId: z2.string().min(1)
}).strict();
var ResourceHintSchema = z2.object({
  resourceType: z2.string(),
  resourceId: z2.string(),
  requestedCapabilities: z2.array(JsonValueSchema).min(1)
}).strict();
var BootstrapSchema = z2.object({
  schema: z2.literal(TRANSCRIPT_SHARE_BOOTSTRAP_SCHEMA),
  policyId: z2.string(),
...
var ChallengeSchema = z2.object({
  schema: z2.literal(POLICY_ENGINE_CHALLENGE_RESPONSE_SCHEMA),
  challengeId: z2.string(),
  policyId: z2.string(),
  audience: z2.string(),
  nonce: z2.string().min(16),
  challengeExpiresAt: Rfc3339Schema,
  acceptedSuites: z2.array(z2.string()).min(1),
  requestedCapabilitiesTemplate: z2.array(JsonValueSchema).optional(),
  signature: SignatureSchema
}).strict();
var ChallengeResponseSchema = z2.object({ challenge: ChallengeSchema }).strict();
var DenialSchema = z2.object({
  schema: z2.literal(POLICY_ENGINE_DENIAL_SCHEMA),
  code: z2.enum(POLICY_ENGINE_GRANT_PRESENTATION_DENIAL_CODES),
  message: z2.string().optional()
}).strict();
var ErrorEnvelopeDenialSchema = z2.object({
  error: z2.object({
    code: z2.enum(POLICY_ENGINE_GRANT_PRESENTATION_DENIAL_CODES),
    message: z2.string().optional()
  }).strict()
...
var CapabilitySchema = z2.object({
  service: z2.enum(["tinycloud.kv", "tinycloud.sql", "tinycloud.vfs"]),
  space: z2.string(),
  path: z2.string(),
  actions: z2.array(z2.string()).min(1),
  caveats: JsonValueSchema.optional()
}).strict();
var WireDelegationSchema = z2.object({
  delegationId: z2.string(),
  issuerDid: z2.string(),
  holderDid: z2.string(),
  policyId: z2.string(),
  capabilities: z2.array(CapabilitySchema).min(1).optional(),
  issuanceId: z2.string().optional(),
  capabilityHashHex: z2.string().optional(),
  revocationMode: z2.literal("refresh_only").optional(),
  issuedAt: Rfc3339Schema,
  expiresAt: Rfc3339Schema,
  terminal: z2.boolean(),
  encoded: z2.string()
}).strict();
var ResolveResponseSchema = z2.object({ delegation: WireDelegationSchema }).strict();
var DelegateReceiptSchema = z2.object({ cid: z2.string().min(1), activated: z2.array(z2.string()), skipped: z2.array(z2.string()) }).strict();
var SqlReadResponseSchema = z2.object({ rows: z2.array(JsonValueSchema) }).strict();
var KvReadResponseSchema = z2.object({ value: JsonValueSchema }).strict();
...
var EnsDataSchema2 = z3.object({
  /** ENS name/domain. */
  domain: z3.string().nullable().optional(),
  /** ENS avatar URL. */
  avatarUrl: z3.string().nullable().optional()
});
var PersistedTinyCloudSessionSchema = z3.object({
  /** The delegation header containing the UCAN */
  delegationHeader: z3.object({
    Authorization: z3.string()
  }),
  /** The delegation CID */
  delegationCid: z3.string(),
  ...
  spaces: z3.record(z3.string(), z3.string()).optional(),
  /** The verification method DID */
  verificationMethod: z3.string()
});
var PersistedSessionDataSchema = z3.object({
  /** User's Ethereum address */
  address: z3.string().regex(ethereumAddressPattern, "Invalid Ethereum address"),
  /** EIP-155 Chain ID */
  chainId: z3.number().int().positive(),
  /** Session key in JWK format (stringified) */
  sessionKey: z3.string(),
   ...
   */
  tinycloudHosts: z3.array(z3.string()).optional()
});
var TinyCloudSessionSchema = z3.object({
  /** User's Ethereum address */
  address: z3.string().regex(ethereumAddressPattern, "Invalid Ethereum address"),
  /** EIP-155 Chain ID */
  chainId: z3.number().int().positive(),
...
var JWKSchema = z4.object({
  /** Key type (e.g., "EC", "RSA", "OKP") */
  kty: z4.string(),
  /** Curve for EC/OKP keys (e.g., "P-256", "Ed25519") */
  crv: z4.string().optional(),
  /** X coordinate for EC keys, public key for OKP */
  x: z4.string().optional(),
  /** Y coordinate for EC keys */
  y: z4.string().optional(),
  /** Private key value (d parameter) */
  d: z4.string().optional(),
  /** Public exponent for RSA keys */
  e: z4.string().optional(),
  /** Modulus for RSA keys */
  n: z4.string().optional(),
  /** Key ID */
  kid: z4.string().optional(),
  /** Algorithm */
  alg: z4.string().optional(),
  /** Key use (e.g., "sig", "enc") */
  use: z4.string().optional(),
  /** Key operations (e.g., ["sign", "verify"]) */
  key_ops: z4.array(z4.string()).optional()
});
var KeyTypeSchema = z4.enum(["main", "session", "ingested"]);
var KeyInfoSchema = z4.object({
...
var DelegationErrorSchema = z4.object({
  /** Error code for programmatic handling */
  code: z4.string(),
  /** Human-readable error message */
  message: z4.string(),
  /** The service that produced the error */
  service: z4.literal("delegation"),
  /** Original error if wrapping another error */
  cause: z4.instanceof(Error).optional(),
...
var DelegationSchema = z4.object({
  ...
  parentCid: z4.string().optional(),
  /** Whether sub-delegation is allowed */
  allowSubDelegation: z4.boolean().optional(),
  /** Authorization header (UCAN bearer token) */
  authHeader: z4.string().optional()
});
var CapabilityEntrySchema = z4.object({
...
var CreateDelegationParamsSchema = z4.object({
  /** DID of the delegate (the party receiving the delegation) */
  delegateDID: z4.string(),
  /** Resource path this delegation grants access to */
  path: z4.string(),
  ...
  disableSubDelegation: z4.boolean().optional(),
  /** Optional statement for the SIWE message */
  statement: z4.string().optional()
});
var DelegationChainSchema = z4.array(DelegationSchema);
var DelegationChainV2Schema = z4.object({
  /** The root delegation from the original authority */
  root: DelegationSchema,
  /** Intermediate delegations in the chain (may be empty) */
  chain: z4.array(DelegationSchema),
  /** The final delegation to the current user */
  leaf: DelegationSchema
});
var DelegationDirectionSchema = z4.enum(["granted", "received", "all"]);
var DelegationFiltersSchema = z4.object({
  ...
  actions: z4.array(z4.string()).optional(),
  /** Include revoked delegations */
  includeRevoked: z4.boolean().optional(),
  /** Filter by delegator DID */
  delegator: z4.string().optional(),
  /** Filter by delegatee DID */
  delegatee: z4.string().optional(),
  /** Only include delegations valid at this time */
  validAt: z4.coerce.date().optional(),
  /** Maximum number of results to return */
  limit: z4.number().optional(),
  /** Cursor for pagination */
  cursor: z4.string().optional()
});
var SpaceOwnershipSchema = z4.enum(["owned", "delegated"]);
var SpaceInfoSchema = z4.object({
  /** Space identifier */
  id: z4.string(),
...
var ShareSchemaSchema = z4.enum(["base64", "compact", "ipfs"]);
var ShareLinkSchema = z4.object({
...
var DelegationApiResponseSchema = z4.object({
  ...
  cid: z4.string().optional()
});
var DelegatedResourceSchema = z4.object({
...
var CreateDelegationWasmParamsSchema = z4.object({
   ...
   * ```
   */
  abilities: z4.record(z4.string(), z4.record(z4.string(), z4.array(z4.string()))),
  /** Expiration time in seconds since Unix epoch */
  expirationSecs: z4.number(),
  /** Optional not-before time in seconds since Unix epoch */
  notBeforeSecs: z4.number().optional()
});
var CreateDelegationWasmResultSchema = z4.object({
  /** Base64url-encoded UCAN delegation */
  delegation: z4.string(),
...
var ServerDelegationsResponseSchema = z5.record(
  z5.string(),
  ServerDelegationInfoSchema
);
var ServerOwnedSpaceSchema = z5.object({
  /** Space identifier */
  id: z5.string(),
...
var ServerOwnedSpacesResponseSchema = z5.array(ServerOwnedSpaceSchema);
var ServerCreateSpaceResponseSchema = z5.object({
  /** Space identifier */
  id: z5.string(),
  /** Space name */
  name: z5.string(),
...
var ServerSpaceInfoResponseSchema = z5.object({
  /** Space identifier */
  id: z5.string(),
  /** Space name (optional) */
  name: z5.string().optional(),
  ...
  owner: z5.string(),
  /** Ownership type */
  type: z5.enum(["owned", "delegated"]).optional(),
  /** Permissions the user has in this space */
...
var AccountService = class {
  constructor(config) {
    ...
    this.index = {
        ...
        return ok3({ database: ACCOUNT_INDEX_DB });
      },
      rebuild: async () => {
        const dbResult = this.accountDb();
        if (!dbResult.ok) return dbResult;
...
var EncodedShareDataSchema = z6.object({
...
var SharingServiceConfigSchema = z6.object({
   ...
   */
  createDelegation: z6.unknown().refine((val) => val === void 0 || typeof val === "function", {
    message: "Expected a createDelegation function or undefined"
  }).optional(),
  /**
   * WASM function for client-side delegation creation.
   */
  createDelegationWasm: z6.unknown().refine((val) => val === void 0 || typeof val === "function", {
    message: "Expected a createDelegationWasm function or undefined"
  }).optional(),
  /**
   * Path prefix for KV operations.
   */
  pathPrefix: z6.string().optional(),
  /**
   * Session expiry time.
   */
  sessionExpiry: z6.date().optional(),
  /**
   ...
   */
  onRootDelegationNeeded: z6.unknown().refine((val) => val === void 0 || typeof val === "function", {
...
// src/index.ts
import {
  ACCOUNT_INDEX_SCHEMA as ACCOUNT_INDEX_SCHEMA2,
  BOOTSTRAP_ALLOWLIST,
  ...
  ENCRYPTION,
  SPACE,
  CAPABILITY_REGISTRY as CAPABILITY_REGISTRY2
...
import { bases as bases2 } from "multiformats/basics";
import { verifyMessage as verifyMessage2 } from "viem";
...
function verifyDidKeyEd25519Signature(did, payload, signature) {
  const publicKey = ed25519PublicKeyFromDidKey2(did);
  return ed255192.verify(signature, payload, publicKey);
}
function ed25519PublicKeyFromDidKey2(did) {
  ...
  const bytes = bases2.base58btc.decode(identifier);
  if (bytes.length === 34 && bytes[0] === 237 && bytes[1] === 1) {
    return bytes.slice(2);
  }
...
export {
  ACCOUNT_INDEX_SCHEMA2 as ACCOUNT_INDEX_SCHEMA,
  ...
  BOOTSTRAP_SPACE_NAMES,
  CAPABILITIES,
  CAPABILITY_REGISTRY2 as CAPABILITY_REGISTRY,
