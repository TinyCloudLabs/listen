...
const FAKE_SERVICE_ACCOUNT = {
  ...
  client_email: "listen@my-project-123.iam.gserviceaccount.com",
...
describe("parsePubSubConfig", () => {
  ...
  it("returns config when both env vars are set", () => {
    ...
    expect(config!.serviceAccountEmail).toBe("listen@my-project-123.iam.gserviceaccount.com");
...
describe("ensurePubSubInfra", () => {
  ...
  it.skip("creates topic and push subscription with correct params", async () => {
    ...
    expect(pubsub.createTopic).toHaveBeenCalledWith("listen-events");

    // Subscription created on the topic
    expect(pubsub.topic).toHaveBeenCalledWith("listen-events");
    ...
    expect(subCall.name).toBe("listen-push");
